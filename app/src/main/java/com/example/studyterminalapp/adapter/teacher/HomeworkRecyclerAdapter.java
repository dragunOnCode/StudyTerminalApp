...
public class HomeworkRecyclerAdapter extends RecyclerView.Adapter<HomeworkRecyclerAdapter.MyViewHolder> {
    ...
    public HomeworkRecyclerAdapter(Context context, List<Homework> list) {
    ...
    public MyViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        ...
        inflater = LayoutInflater.from(context).inflate(R.layout.homework_item, parent, false);
    ...
    public void onBindViewHolder(MyViewHolder holder, int position) {
        ...
        holder.tvHomeworkName.setText(homework.getHomeworkName());
        String openDate = df.format(homework.getOpenDate());
        holder.tvOpenDate.setText(openDate.substring(0, 10));
        String deadline = df.format(homework.getDeadline());
        holder.tvDeadline.setText(deadline.substring(0, 10));
        holder.tvFinishedCount.setText(homework.getFinishedCount().toString());
        holder.tvStudentCount.setText(homework.getStudentCount().toString());
        holder.tvCorrectCount.setText(homework.getCorrectCount().toString());
        holder.tvFinishedCount2.setText(homework.getFinishedCount().toString());
        ...
        holder.tvOpenDateStatus.setText(openDateStatus);
        int pos = position;
    ...
    class MyViewHolder extends RecyclerView.ViewHolder {
        TextView tvHomeworkName, tvOpenDate, tvDeadline, tvOpenDateStatus, tvFinishedCount, tvStudentCount,
                tvCorrectCount, tvFinishedCount2;

        public MyViewHolder(View itemView) {//这个view参数就是recyclerview子项的最外层布局
            super(itemView);
            //可以通过findViewById方法获取布局中的TextView
            tvHomeworkName = (TextView) itemView.findViewById(R.id.tv_homework_name);
            tvOpenDate = itemView.findViewById(R.id.tv_open_date);
            tvDeadline = itemView.findViewById(R.id.tv_deadline);
            tvFinishedCount = itemView.findViewById(R.id.tv_finished_count);
            tvStudentCount = itemView.findViewById(R.id.tv_student_count);
            tvOpenDateStatus = itemView.findViewById(R.id.tv_open_date_status);
            tvCorrectCount = itemView.findViewById(R.id.tv_correct_count);
            tvFinishedCount2 = itemView.findViewById(R.id.tv_finished_count2);
            itemView.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View view) {
                    Toast.makeText(context, tvHomeworkName.getText(), Toast.LENGTH_SHORT).show();
                }
            });
        }
    }
