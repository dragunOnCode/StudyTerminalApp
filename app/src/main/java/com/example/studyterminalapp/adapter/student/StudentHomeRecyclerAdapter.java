...
public class StudentHomeRecyclerAdapter extends RecyclerView.Adapter<StudentHomeRecyclerAdapter.MyViewHolder>{
    private Context context;
    private List<HomeClassBean> list;
    private View inflater;
    private OnItemClickListener mOnItemClickListener;

    //构造方法，传入数据,即把展示的数据源传进来，并且复制给一个全局变量，以后的操作都在该数据源上进行
    public StudentHomeRecyclerAdapter(Context context, List<HomeClassBean> list) {
    ...
    public MyViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        ...
        inflater = LayoutInflater.from(context).inflate(R.layout.home_class_item, parent, false);
    ...
    public void onBindViewHolder(MyViewHolder holder, int position) {
        //将数据和控件绑定
        HomeClassBean homeClassBean = list.get(position);
        holder.tvClassName.setText(homeClassBean.getClassName());
        holder.tvSubject.setText(homeClassBean.getCourseName());
        holder.tvGrade.setText(homeClassBean.getGrade());
        holder.tvSchool.setText(homeClassBean.getSchoolName());
        holder.tvStudentNum.setText("");
        int pos = position;
    ...
    class MyViewHolder extends RecyclerView.ViewHolder {
        TextView tvClassName, tvSchool, tvSubject, tvGrade, tvStudentNum;
